...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            for s in self.s_levels[level]:
                for p in node_a.prenodes:
                    if s == p:
                        s.children.add(node_a)
                        node_a.parents.add(s)
            if set(node_a.prenodes) < set(self.s_levels[level]) :
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[a_level]:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ret = False
        for eff in node_a1.action.effect_add:
           for eff2 in node_a2.action.precond_neg:
               if eff == eff2:
                   return True
        for eff in node_a1.action.effect_rem:
           for eff2 in node_a2.action.precond_pos:
               if eff == eff2:
                   return True
        for eff in node_a2.action.effect_add:
           for eff2 in node_a1.action.precond_neg:
               if eff == eff2:
                   return True
        for eff in node_a2.action.effect_rem:
           for eff2 in node_a1.action.precond_pos:
               if eff == eff2:
                   return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for i in range(len(self.s_levels)):
            level_sum += i
            s_lits = [s.literal for s in self.s_levels[i]]
            if set(self.problem.goal) <= set(s_lits):
                return level_sum
        return level_sum
